    <section id="products" class="products-section">
        <div class="section-title">
            <h2>Our <span class="title-gradient">Products</span></h2>
            <div class="title-underline"></div>
            <p class="section-subtitle">We bring the sweetness to every occasion</p>
        </div>

        <!-- Products Container with Side Scroll -->
        <div class="products-scroll-container">
            <div class="products-scroll-wrapper">

                <!-- Product 1 -->
                <div class="product-card-new">
                    <div class="product-image-container">
                        <img src="./image/chips-galore-sandwich.webp" alt="Chips Galore Sandwich" class="product-img">
                    </div>
                    <div class="product-info">
                        <h3 class="product-title">Chips Galore Sandwich</h3>
                        <p class="product-description">Delicious ice cream sandwich with chocolate chip cookies</p>
                        <button class="btn-booking">Booking Now</button>
                    </div>
                </div>

                <!-- Product 2 -->
                <div class="product-card-new">
                    <div class="product-image-container">
                        <img src="./image/CookiesCream.webp" alt="Cookies & Cream" class="product-img">
                    </div>
                    <div class="product-info">
                        <h3 class="product-title">Cookies & Cream</h3>
                        <p class="product-description">Classic cookies and cream ice cream bar</p>
                        <button class="btn-booking">Booking Now</button>
                    </div>
                </div>

                <!-- Product 3 -->
                <div class="product-card-new">
                    <div class="product-image-container">
                        <img src="./image/OriginalBombPop.webp" alt="Original Bomb Pop" class="product-img">
                    </div>
                    <div class="product-info">
                        <h3 class="product-title">Original Bomb Pop</h3>
                        <p class="product-description">Red, white & blue frozen pop - a true American classic</p>
                        <button class="btn-booking">Booking Now</button>
                    </div>
                </div>

                <!-- Product 4 -->
                <div class="product-card-new">
                    <div class="product-image-container">
                        <img src="./image/strawberry-shortcake.webp" alt="Strawberry Shortcake" class="product-img">
                    </div>
                    <div class="product-info">
                        <h3 class="product-title">Strawberry Shortcake</h3>
                        <p class="product-description">Strawberry ice cream with crumbly cake coating</p>
                        <button class="btn-booking">Booking Now</button>
                    </div>
                </div>

                <!-- Product 5 -->
                <div class="product-card-new">
                    <div class="product-image-container">
                        <img src="./image/vanilla-big-dipper.webp" alt="Vanilla Big Dipper" class="product-img">
                    </div>
                    <div class="product-info">
                        <h3 class="product-title">Vanilla Big Dipper</h3>
                        <p class="product-description">Vanilla ice cream cone dipped in chocolate & peanuts</p>
                        <button class="btn-booking">Booking Now</button>
                    </div>
                </div>

            </div>
        </div>

        <!-- Scroll Indicator Dots -->
        <div class="products-scroll-dots" id="productsDots"></div>
    </section>
